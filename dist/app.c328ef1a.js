...
var NEWS_URL = "https://api.hnpwa.com/v0/news/1.json"; // 바뀔 가능성이 있는 변수는 뺴주는게 좋다 URL

var CONTENT_URL = "https://api.hnpwa.com/v0/item/@id.json";
ajax.open("GET", NEWS_URL, false); // 동기적으로 처리하겠다.

ajax.send(); // 데이터를 가져오겠다
//응답 값을 객체로 바꿔보자 (preview 탭)
// JSON 데이터를 객체로 바꾸는 도구는 'json.parse)이다

var newsFeed = JSON.parse(ajax.response);
var ul = document.createElement("ul"); // 객체화되어 있을 것 (axios에서는 이 기능을 자동으로 처리해줌)

window.addEventListener("hashchange", function () {
  var id = location.hash.substring(1); // 쓰고 싶은 위치값부터 그 이후부터만 나오게
  ...
  console.log(newsContent); // 문자열 함수 중에 값을 대체해주는 replace 함수가 있다
});
...
container.appendChild(content); // 똑같은 코드 반복은 좋지 않다.
